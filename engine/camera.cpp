...
void Camera::render(const Actors& actors) const {
    for (auto actor : actors) {
        if (within_view(actor->get_position()) && actor->alive &&
            actor->is_visible()) {
...
void Camera::render_rect(const Vec& position, int red, int green, int blue,
                         int alpha) const {
...
Vec Camera::world_to_screen(const Vec& position) const {
    // World Coords (cartesian, up is positive y) -> Pixel Coords (image, down
    // is positive y):
...
void Camera::calculate_visibility_limits() {
    Vec num_tiles =
        Vec{graphics.width, graphics.height} / (2 * zoom * tilesize) +
        Vec{1, 1};
...
bool Camera::within_view(const Vec& position) const {
    return min.x <= position.x && position.x <= max.x && min.y <= position.y &&
           position.y <= max.y;
}
